...
- `Lakeraven::Integrations::Supplemental::Attributes` — the UDS supplemental attribute vocabulary (internal code system `https://terminology.lakeraven.com/CodeSystem/uds-supplemental-attribute`): `income_percent_fpl`, `sliding_fee_class`, `payer_category`, `housing_status`, `agricultural_worker_status`, `veteran_status`, `language_barrier` (patient-level) and `visit_service_category` (visit-level). This vocabulary IS the contract adapters normalize into, so downstream SQL-on-FHIR sees one vocabulary regardless of source EHR.
